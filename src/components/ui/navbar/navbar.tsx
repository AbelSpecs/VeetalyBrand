...
export default function Navbar() {
  ...
  const username = isAuthenticated() ? JSON.parse(sessionStorage.getItem('name')!) : 'Inicia Sesión';

  const handleMenu = () => {
    isAuthenticated() ? setHideMenu(prev => !prev) : router.push('/auth');
  }
        ...
        <div className="lg:flex lg:items-center lg:w-auto justify-end">
          <div className="flex gap-5">
            <div className="flex flex-col text-white items-center justify-center">
              <p className="font-normal text-xl tracking-tight text-black">Hola! {username}</p>
            </div>
            <Image src="https://img.logoipsum.com/295.svg" width={36} height={36} alt="logo" className="cursor-pointer shadow rounded-full max-w-full h-auto align-middle border-none" onClick={handleMenu}/>
            {
